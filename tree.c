...
FileNode* createNode(const char *name, int isDir) {
    ...
    if (!node->name) { perror("strdup"); exit(EXIT_FAILURE); }
    node->isDir = isDir;
...
FileNode* buildTree(const char *path) {
    ...
    // 11. 释放临时数组，返回当前目录结点
    FileNode* buildTree(const char *path, int depth) {
    // 深度限制检查
    if (g_maxDepth != -1 && depth > g_maxDepth) {
        return NULL;
    }

    DIR *dir = opendir(path);
    if (!dir) {
        // 无法打开目录，返回 NULL 但上层会跳过
        return NULL;
    }
    ...
    while ((entry = readdir(dir)) != NULL) {
        ...
        if (child) {
            ...
            if (!children) { perror("realloc"); exit(EXIT_FAILURE); }
...
void printTree(FileNode *node, const char *prefix, int isLast) {
    ...
    //     递归调用 printTree
    if (!node) return;
    fprintf(g_outFile, "%s", prefix);
    fprintf(g_outFile, "%s", isLast ? "`-- " : "|-- ");
    fprintf(g_outFile, "%s", node->name);
    if (node->isDir) fprintf(g_outFile, "/");
    fprintf(g_outFile, "\n");
